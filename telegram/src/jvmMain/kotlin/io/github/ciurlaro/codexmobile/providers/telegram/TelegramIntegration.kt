...
class TelegramIntegration(
    private val store: TelegramSessionStorage,
    private val credentials: TelegramCredentials,
    private val clientInfo: TelegramClientInfo,
) : TelegramClient {
    ...
    @Synchronized
    fun prepareRemoval(): TelegramRemovalOutcome {
        authentication?.cancel()
        val authority = runCatching { store.authority() }.getOrElse {
            return TelegramRemovalOutcome(false, "Telegram session authority is unreadable; removal was not started")
        }
        val priorUncertainty = store.remoteRevocationUnconfirmed()
        if (authority == null) {
            return if (store.clearAll()) TelegramRemovalOutcome(true, priorUncertainty.warning())
            else TelegramRemovalOutcome(false, "Telegram local data could not be removed")
        }
        val wasActive = activeSession != null
        val session = try {
            activeSession ?: openSession(authority, store.sessionDirectory(authority))
        } catch (_: Exception) {
            return TelegramRemovalOutcome(false, "Telegram remote logout could not be verified")
        }
        val revoked = try {
        ...
        if (!wasActive) session.close()
        return if (store.clearAll()) TelegramRemovalOutcome(true, priorUncertainty.warning())
        else TelegramRemovalOutcome(false, "Telegram logout succeeded, but local cleanup needs retry")
    }
