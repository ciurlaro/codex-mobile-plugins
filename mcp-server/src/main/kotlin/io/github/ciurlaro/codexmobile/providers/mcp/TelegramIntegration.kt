...
internal class TelegramIntegration(
    ...
    private val store = TelegramSessionStore(root)
    private val apiId = System.getenv("TELEGRAM_API_ID")?.toIntOrNull()
    private val apiHash = System.getenv("TELEGRAM_API_HASH").orEmpty()
    ...
    override fun disconnect(): TelegramDisconnectResult {
        ...
        if (store.authority() == null) {
            closeActive()
            return if (store.clearAll()) TelegramDisconnectResult.CONFIRMED
            else TelegramDisconnectResult.INDETERMINATE
        }
        val remoteConfirmed = runCatching { healthySession().logOut() }.getOrDefault(false)
        closeActive()
        val localCleared = store.clearAll()
        return if (remoteConfirmed && localCleared) TelegramDisconnectResult.CONFIRMED
        else TelegramDisconnectResult.INDETERMINATE
    }
    ...
    private fun openSession(authority: TelegramAuthority, directory: File) = TdLibSession(
        ...
        apiHash = apiHash,
        applicationVersion = "Codex plugin TDLib 1.8.66",
    )
